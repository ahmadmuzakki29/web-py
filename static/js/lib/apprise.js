...
function set_overlay(args,n){
	...
	// ini untuk website pemda
	if($("#container").doesExist()){
		var margin = $("#main").width() - $("#container").width();
		$("#main").css({marginLeft:(margin/2)});
	}
...
function notif(title,callback,confirm){
	...
	$('.notifInner').append('<div class="aNotifButtons" style="text-align:right;width:100%;"></div>');
	$('.aNotifButtons').append('<button id="btn_ok_notif" style="width:70px;" value="ok"  >OK</button>');
	if(confirm){$('.aNotifButtons').append('<button style="width:70px;" value="cancel" >Batal</button>');}
	$("#btn_ok_notif").focus();
